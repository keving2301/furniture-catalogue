      ...
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on" >New Bedroom Set</v-btn>
          </template>
          <v-card>
...
export default {
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1 ? "New Bedroom Set" : "Edit Bedroom Set";
    },
  },
  ...
  methods: {
    ...mapActions({
      addBedroomSets: 'bedroomSets/addBedroomSets',
      updateBedroomSets: 'bedroomSets/updateBedroomSets',
      removeBedroomSets: 'bedroomSets/removeBedroomSets'
    }),
    async initialize() {
        this.loading = true;
        try {
          await this.loadBedroomSets();
        } catch (e) {
          console.error(e);
        }
        this.loading = false;
    ...
    editItem(item) {
      this.editedIndex = this.bedroomSets.indexOf(item);
    ...
    async deleteItem(item) {
      ...
      if (confirm("Are you sure you want to delete this item?")) {
        this.loading = true;
        try {
          await this.removeBedroomSets(item);
    ...
    async save() {
      ...
      if (this.editedIndex > -1) {
        this.loading = true;
        try {
          await this.updateBedroomSets({index: this.editedIndex, bedroomSet: this.editedItem});
          this.loading = false;
          this.close();

          this.snack = true
          this.snackColor = 'success'
          this.snackText = 'Bedroom Set has been updated'
          ...
          this.snack = true
          this.snackColor = 'error'
          this.snackText = 'Bedroom Set could not be updated'
        ...
        this.loading = true;
        try {
          await this.addBedroomSets(this.editedItem);
          this.loading = false;
          this.close();

          this.snack = true
          this.snackColor = 'success'
          this.snackText = 'Bedroom Set added'
          ...
          this.snack = true
          this.snackColor = 'error'
          this.snackText = 'Bedroom Set could not be created'
